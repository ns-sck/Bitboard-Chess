...
std::vector<Move> WhiteQueen::generate_moves(uint64_t position, uint64_t team, uint64_t enemy) {
...
uint64_t WhiteQueen::get_attacked_pos(uint64_t position, uint64_t team, uint64_t enemy) {
    uint64_t attack = 0;
    uint64_t p = position;
    while (p) {
        int sq = get_lsb_idx(p);
...
char WhiteQueen::get_symbol() const {
    return 'Q';
}

int WhiteQueen::get_value() const {
    return 9;
} 